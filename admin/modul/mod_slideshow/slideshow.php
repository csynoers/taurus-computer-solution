            ...
            foreach ( read_file('../json/slideshow.json') as $key => $value) {
                $rows_html .= "
                    <div class='col-xs-12 col-sm-6'>
                        ...
                        <hr>
                        <form method=POST enctype='multipart/form-data' action='{$aksi}?module=slideshow&act=update&id={$key}'>
                            <input type='file' name='fupload' required>
                            <button type='submit' class='btn btn-primary'>Update</button>
                        </form>
                        <button type='button' onclick=\"window.location.href='{$aksi}?module=slideshow&act=delete&id={$key}';\" class='btn btn-danger'>Delete</button>
                    </div>
                ";
            }
